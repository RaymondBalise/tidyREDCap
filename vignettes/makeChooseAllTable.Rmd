...
REDCap exports a "choose all that apply" question into a series of similarly-named, binary indicator variables (i.e., the variables are equal to either "checked" or "unchecked"). For example, the following data represents a sample of responses to a race/ethnicity question with multiple checkboxes.
```{r}
sample_data <- readRDS(file = "./sample_race_ethnicity.rds")
sample_data %>% 
  select(starts_with("race___")) %>% 
  head()
```
...
```{r show_results}
make_choose_all_table(sample_data, "race") 
```
