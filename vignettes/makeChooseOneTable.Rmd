...
The `make_choose_one_table()` function can be used with a factor variable.  It can also be used with a data frame name and the name of the factor inside of that data frame.  The `subset` option, if set to TRUE, will cause the function to subset the label's text and only show the response option (not repeat the question).
