...
# To demonstrate the make_instrument function properly, let's simulate some 
# incomplete records by setting some health instrument values to NA
sample_data_with_nas <- sample_data
# Make records 2 and 5 have completely missing health data
sample_data_with_nas[c(2, 5), c("height", "weight", "bmi", "comments", "mugshot", "health_complete")] <- NA

sample_data_with_nas %>% 
  select(
...
```{r}
make_instrument(sample_data_with_nas, "name_first", "demographics_complete") %>% 
  knitr::kable()
```
...
```{r}
make_instrument(sample_data_with_nas, "height", "health_complete") %>% 
  knitr::kable()
```
