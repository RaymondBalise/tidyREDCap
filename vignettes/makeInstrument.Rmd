...
REDCap exports longitudinal projects with one record (a line of data) per assessment (typically 1 line per day).  This works well when every instrument/questionnaire is given at every assessment, but for projects that have different instruments/questionnaires given on different days, REDCap exports blanks (the value NA).  For example, in the Nachos for Anxiety project, three instruments were used and they each had a different administration schedule. Subjects' anxiety was assessed at baseline with the Generalized Anxiety Disorder 7-item (GAD-7) scale. Every day, it was assessed with the Hamilton Anxiety Scale (HAM-A), but the Nacho Craving Index was administered only at the baseline and at the end of the study. 

![](./schedule.png)

The instruments that are not assessed every day appear as entirely blank questionnaires when the data is exported.  For example, the NCI is shown as missing for the Day 1, Day 2 and Day 3 anxiety assessment visits where it was not administered. 
...
Pass the `make_instrument()` function the name of a dataset and the names of the first and last variables in an instrument and it will return a table that has the not blank records for the instrument.  For example, to extract the enrollment/consent instrument:
